...
/// Fetch the canned (or deterministically fabricated) Set backing one Mapping.
/// The mock never fails — a real Provider's fallible `fetch` collapses here to a
/// total function over the seeded data.
pub(crate) fn fetch(mapping: &Mapping) -> SecretShape {
    seeded(&mapping.secret_id).unwrap_or_else(|| fallback(&mapping.secret_id, &mapping.environment))
}
...
mod tests {
    ...
    fn seeded_payments_has_the_mockup_entries() {
        let prod = fetch(&map("payments/prod", "prod"));
    ...
    fn seeded_github_app_id_aligned_replica_is_gap_stripe_drifts() {
        let prod = fetch(&map("payments/prod", "prod"));
        let stg = fetch(&map("payments/staging", "staging"));
        assert_eq!(
    ...
    fn fallback_is_deterministic_and_mixes_states() {
        let a = fetch(&map("auth/prod", "prod"));
        let b = fetch(&map("auth/prod", "prod"));
        assert_eq!(entries(&a), entries(&b), "same input → same shape");

        let prod = fetch(&map("auth/prod", "prod"));
        let stg = fetch(&map("auth/staging", "staging"));
        assert_eq!(
